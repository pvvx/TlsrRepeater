    ...
    while (pData < pScene->extField + pScene->extFieldLen) {
        ...
        if (clusterID == ZCL_CLUSTER_GEN_LEVEL_CONTROL) {
            if (extLen >= 1) {
                ...
                app_levelCb(pAddrInfo, ZCL_CMD_LEVEL_MOVE_TO_LEVEL, &moveToLevel);
                extLen--;
            }
        }else
#endif
        ...
        if (clusterID == ZCL_CLUSTER_LIGHTING_COLOR_CONTROL) {
            if (extLen >= 13) {
                ...
                if (move2Color.colorX || move2Color.colorY) {
                    app_colorCtrlCb(pAddrInfo, ZCL_CMD_LIGHT_COLOR_CONTROL_MOVE_TO_COLOR, &move2Color);
                } else
#endif
                {
                    ...
                    if (move2EnhHueAndSat.enhancedHue || move2EnhHueAndSat.saturation) {
                        app_colorCtrlCb(pAddrInfo, ZCL_CMD_LIGHT_COLOR_CONTROL_ENHANCED_MOVE_TO_HUE_AND_SATURATION, &move2EnhHueAndSat);
                    }

                    if (colorLoopSet.action || colorLoopSet.direction || colorLoopSet.time) {
                        app_colorCtrlCb(pAddrInfo, ZCL_CMD_LIGHT_COLOR_CONTROL_COLOR_LOOP_SET, &colorLoopSet);
                    }
#endif
                    ...
                    if (move2CT.colorTemperature) {
                        app_colorCtrlCb(pAddrInfo, ZCL_CMD_LIGHT_COLOR_CONTROL_MOVE_TO_COLOR_TEMPERATURE, &move2CT);
                    }
#endif
                }
            }
 ...
 * @return  None
 */
static void app_sceneStoreReqHandler(zcl_sceneEntry_t *pScene)
{
    u8 extLen = 0;
    ...
    if (pAddrInfo->dstEp == APP_ENDPOINT) {
        if (pAddrInfo->dirCluster == ZCL_FRAME_CLIENT_SERVER_DIR) {
            ...
            case ZCL_CMD_SCENE_STORE_SCENE:
                app_sceneStoreReqHandler((zcl_sceneEntry_t *)cmdPayload);
                break;
